...
func CheckJwt(tokenStr string) (*StudentToken, error) {
	token, err := jwt.ParseWithClaims(tokenStr, &StudentToken{}, func(token *jwt.Token) (interface{}, error) {
	...
	claims, ok := token.Claims.(*StudentToken)
	if !ok {
